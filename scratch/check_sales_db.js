...
async function check() {
  ...
  const url = `${supabaseUrl}/rest/v1/sales_sync?select=*&user_id=eq.${userId}&order=created_at.desc&limit=50`;
  ...
  if (!response.ok) {
    console.error('Error fetching sales:', response.status, await response.text());
    return;
  }

  const data = await response.json();
  console.log('--- RECENT SALES 0002 IN DATABASE ---');
